...
static void drawTerrios(ofFbo &fbo,ofEasyCam &cam, double sepalation, double length, int w, int h){
    ofEnableAlphaBlending();
    ofEnableBlendMode(OF_BLENDMODE_ALPHA);
    ofEnableDepthTest();
    fbo.begin();
    ...
    ofSetColor(100);
        cam.begin();
            ...
            ofDrawAxis(100);
        for(int i = 0; i < w * h; i++){
                double xf = ( sepalation + length ) * ( i % w ) + length;
                double xn = ( sepalation + length ) * ( i % w ) ;
                double yf = ( sepalation + length ) * ( (h -1 ) - int(i / h))+ length;
                double yn = ( sepalation + length ) * ( (h -1 ) - int(i / h)) ;
            ofDrawRectangle(xn,yn,0,length,length);
            /*
                markerCorners.push_back(cv::Point3d(xn,yf,0));
                markerCorners.push_back(cv::Point3d(xf,yf,0));
                markerCorners.push_back(cv::Point3d(xf,yn,0));
                markerCorners.push_back(cv::Point3d(xn,yn,0));
             */
        }
        cam.end();
    fbo.end();
...
static void setCameraParameter(ofCamera &camera, cv::Mat &intrinscic, float camWidth, float camHeight, float far, float near){
